...
CREATE TABLE IF NOT EXISTS rentals (
  ...
  type TEXT NOT NULL CHECK (type IN ('rental', 'demo')), -- rental or demo
  price_monthly DECIMAL(10, 2), -- monthly rental price
  price_daily DECIMAL(10, 2), -- daily demo price
  original_price DECIMAL(10, 2), -- purchase price for reference
  description TEXT,
  ...
  min_rental_period INTEGER DEFAULT 3, -- minimum rental period in months
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Create rental_requests table (for rental/demo applications)
CREATE TABLE IF NOT EXISTS rental_requests (
  ...
  rental_period TEXT, -- for rental: "3개월", "6개월", etc
  preferred_date DATE,
  message TEXT,
...
CREATE POLICY "Only admins can insert rentals" ON rentals
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      AND profiles.role IN ('admin', 'hello_admin')
    )
  );
